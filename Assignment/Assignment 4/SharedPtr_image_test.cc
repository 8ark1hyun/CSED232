...
void compute_mystyle(const Image<RGBf>& img, Image<RGBf>& mystyle)
{
	mystyle = Image<RGBf>(img.width(), img.height(), RGBf(.0f, .0f, .0f));
	for (int y = 0; y < img.height() - 6; y++) {
		for (int x = 0; x < img.width() - 6; x++) {
			for (int ch = 0; ch < 3; ch++) {
				float dx = img[y][x + 1][3 - ch] - img[y][x][3 - ch];
				float dy = img[y + 1][x][3 - ch] - img[y][x][3 - ch];
				mystyle[y][x][0] = std::sqrt(dx * dx + dy * dy) * 5;
				mystyle[y][x + 3][1] = std::sqrt(dx * dx + dy * dy) * 5;
				mystyle[y][x + 6][2] = std::sqrt(dx * dx + dy * dy) * 5;
			}
		}
	}
}
...
int main()
{
	...
	Image<RGBf> mystyle;
	compute_mystyle(imgf, mystyle);
	convert_pixel_type(mystyle, img);
	save_bmp("mystyle.bmp", img);
